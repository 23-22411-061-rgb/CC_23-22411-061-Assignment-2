...
output "configuration_guide" {
  value = <<-EOT
    ...
    3. Update backend IPs in upstream block:
       - BACKEND_IP_1: ${join(", ", module.backend_servers["web-1"].private_ip)}
       - BACKEND_IP_2: ${join(", ", module.backend_servers["web-2"].private_ip)}
       - BACKEND_IP_3: ${join(", ", module.backend_servers["web-3"].private_ip)}
    ...
    ${join("\n    ", [
      ...
      "- ${name}: public IPs: ${join(", ", server.public_ip)}, private IPs: ${join(", ", server.private_ip)}"
    ])}
